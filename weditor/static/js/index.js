...
window.vm = new Vue({
  ...
  data: {
    ...
    checkpointLabels:[
      "activity", "fuzzyScreen", "install", "uninstall",
      "click", "textbox", "exact",  "exclude"
    ],
    totalCheckpointData: [],
    // checkpointData: {
    //   "activity": "",
  ...
  computed: {
    ...
    annotationDis() {
      // 检查数组中是否所有的状态都是false
      return this.buttonStates.every(state => state === false);
    },
 ...
 toggleButton: function(i) {
    // 这里需要添加判断，只有当按钮不需要 nodeSelected 或者 nodeSelected 为真时，才切换状态
    // if (i < 4 && !this.nodeSelected) {
    //   return; // 如果是前四个按钮，并且没有节点被选中，则不执行任何操作
    // }
    this.$set(this.buttonStates, i, !this.buttonStates[i]); // Ensure the change is reactive
 ...
 addAnnotation: function() {
  const labels = this.checkpointLabels;
  let checkpoint = {};
  labels.forEach((label, i) => {
    if (this.buttonStates[i] === true) { // Make sure to use the correct index variable here
      if (label === 'activity') {
        checkpoint[label] = 0;
        // checkpoint[label] = `${label}<${0}>`;
        // this.checkpointData[label] = `${label}<${0}>`;
      } else if (label === 'fuzzyScreen') {
        checkpoint[label] = -1;
        // checkpoint[label] = `fuzzy<${-1}>`;
        // this.checkpointData[label] = `fuzzy<${-1}>`;
      } else if (label === 'install' || label === 'uninstall') {
        checkpoint[label] = this.nodeSelected.package;
        // checkpoint[label] = `${label}<${this.nodeSelected.package}>`;
        // this.checkpointData[label] = `${label}<${this.nodeSelected.package}>`;
        // Add logic here for 'install' and 'uninstall' if needed
      } else if (label === 'textbox') {
        checkpoint[label] = this.elemXPathLite;
        // checkpoint[label] = `fuzzy<${this.elemXPathLite}>`;
        // checkpoint[label] = `fuzzy<${this.elemXPathFull}>`;
        // this.checkpointData[label] = `fuzzy<${this.elemXPathFull}>`;
      } else {
        checkpoint[label] =  this.elemXPathLite;
        // this.checkpointData[label] = `${label}<${this.elemXPathLite}>`;
  ...
  this.totalCheckpointData.push(checkpoint);
  console.log(this.totalCheckpointData);

  this.resetButtonStates();
 },

 updateAndSendAnnotation: function() {
  this.annotating = true;
  // const labels = checkpointLabels;
  // labels.forEach((label, i) => {
  //   if (this.buttonStates[i]) { // Make sure to use the correct index variable here
  //     if (label === 'activity') {
  //       this.checkpointData[label] = `${label}<${0}>`;
  //     } else if (label === 'fuzzyScreen') {
  //       this.checkpointData[label] = `fuzzy<${-1}>`;
...
sendDataToBackend: function() {
  ...
  $.ajax({
    url: '/api/AndroidMock/Annotation',
    method: 'POST',
    contentType: 'application/json',
    data: JSON.stringify({
      deviceId: self.deviceId,
      index: self.index,
      data: self.totalCheckpointData,
    }),
    success: function(response) {
      console.log('Data successfully sent to the backend:', response);
      self.totalCheckpointData = []; // 使用self引用
    },
  ...
  getButtonLabel: function(i) {
    return this.checkpointLabels[i];
  },
  decrementIndex: function() {
      if (this.index > 0) {  // Check to prevent index from going negative
          this.index -= 1;
          this.sendIndex();  // Assuming you have a method to fetch data based on the index
          this.dumpHierarchyWithScreen(); 
          this.dumpHierarchyWithScreen(); 
          this.resetButtonStates();
      }
  ...
  sendIndex: function() {
    var self = this;
    $.ajax({
      url: '/api/AndroidMock/update-index',
      method: 'POST',
      contentType: 'application/json',
      data: JSON.stringify({
        index: this.index,
      ...
      success: function(data) {
        ...
        if (!data.success) {
          alert(data.description); // Alert the user if index was adjusted
          self.index = data.index; // Update local index if changed on server
        }
      },
      error: function(xhr, status, error) {
